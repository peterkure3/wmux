...
// surface.
// Usage is `wmux surface [command...]`: the trailing arguments are the
// command to run, and everything else defaults (see args.go) — an ID
// derived from the working directory, the current directory, the
// daemon's default platform. `wmux surface` alone opens a shell.
func cmdSurface(args []string) {
	...
	native := fs.Bool("native", false, "run the command directly on the daemon's OS, no WSL")
	fs.Parse(args)

	dir := resolveCwd(*cwd)
	cmd := resolveCmd(positionalCommand(*command, fs.Args()))
	if cmd == "" {
		cmd = defaultShellCommand(*native)
	}

	req := proto.NewSurfaceRequest{
		ID: resolveSessionID(*id, dir), Cwd: dir, Command: cmd,
	...
	fmt.Fprintf(os.Stderr, "view it with: wmux connect %s\n", info.ID)
	fmt.Println(info.ID)
}
...
//
// Usage is `wmux connect [ID]`. With no ID and exactly one surface
// running, it attaches to that one.
func cmdConnect(args []string) {
	fs := newFlagSet("connect")
	idFlag := fs.String("id", "", "surface session ID (default: the only running surface)")
	fs.Parse(args)

	id := *idFlag
	if id == "" {
		id = resolveSurfaceID("connect", fs.Args())
	}
	...
	select {
	case <-detached:
		fmt.Fprintf(os.Stderr, "\r\n[wmux: detached from %s — it keeps running; reconnect with 'wmux connect %s']\r\n", id, id)
	default:
		if exited {
