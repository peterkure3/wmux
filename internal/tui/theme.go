...
// themeConfigPath is where ThemeSet persists the chosen theme — a
// plain-text file holding just the name, read fresh by
// currentSidebarTheme on every TUI launch. Persisting to disk (rather
// than relying solely on WMUX_THEME) matters because a terminal launched
// from a shortcut or scheduler inherits that launcher's environment, not
// the shell the wmux CLI happened to be invoked from.
...
// ThemeNames lists every known theme, sorted — the CLI's `wmux theme`
// listing, and the set ThemeSet validates against.
...
// ThemeCurrent names the theme that would be used by a TUI started now.
func ThemeCurrent() string { return currentSidebarTheme().name }

// ThemeSet validates name and persists it for the next TUI launch. An
// already-running TUI keeps its look: styles resolve once at startup.
func ThemeSet(name string) error {
